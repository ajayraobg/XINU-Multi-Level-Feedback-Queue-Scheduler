...
void burst_execution(uint32 number_bursts, uint32 burst_duration, uint32 sleep_duration){
	...
	while(proctab[currpid].number_bursts>0){
		...
		while(proctab[currpid].burst_completed != 1)
			;
		
		if(proctab[currpid].number_bursts > 1 && proctab[currpid].sleep_duration > 1)
		...
		if(proctab[currpid].number_bursts>0){
			proctab[currpid].burst_duration = burst_duration;
			proctab[currpid].burst_completed = 0;
		}
	}
	...
	)
{
	...
	prptr->burst_completed = 0;
	prptr->queue_num = 1;
	prptr->alotted_time_used = 0;
